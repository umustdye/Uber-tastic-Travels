...
function cab_price(data) {
    ...
    for (let i = 0; i < final_data.length; ++i) {
        temp = final_data[i].price / final_data[i].distance;
        var temp_array = temp.toString().split(".");
        //PPM = temp.toPrecision(3);
        if (temp_array[0] != 0) {
            PPM = parseFloat(temp.toPrecision(temp_array[0].length + 2));
        }
        else {
            PPM = parseFloat(temp.toPrecision(2));
        }
        if (final_data[i].cab_type == "Uber") {
