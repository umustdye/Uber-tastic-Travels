...
function Parsefhv()
{
    ...
    fileData0 = fs.readFileSync(fileName_1, 'utf8').slice(1);
    ...
    fhv_Obj = []
    fhv_Obj = parseCSVToJSON(fileData0, fhv_names[0], fhv_Obj);
...
function parseCSVToJSON(data, name, fhv)
{
    ...
    rows.forEach(row => {
        ...
        else
        {
            ...
            const data = {"date":date, "time":time, "address": address, "name":name};
            fhv.push(data);
        }
